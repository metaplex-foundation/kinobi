...
export function getDefaultValidatorBagVisitor(): Visitor<ValidatorBag> {
  ...
  const tappedVisitor = tapDefinedTypesVisitor(visitor, (definedTypes) => {
    definedTypeNames = new Set<string>(definedTypes.map((type) => type.name));
  });

  return extendVisitor(tappedVisitor, {
