...
export function renderJavaScriptExperimentalValueNode(value: nodes.ValueNode): {
  ...
  switch (value.kind) {
    case 'list':
    case 'tuple':
      const list = value.values.map((v) =>
        renderJavaScriptExperimentalValueNode(v)
      );
      return {
    ...
    case 'map':
      const map = value.values.map(([k, v]) => {
        const mapKey = renderJavaScriptExperimentalValueNode(k);
        const mapValue = renderJavaScriptExperimentalValueNode(v);
        return {
    ...
    case 'struct':
      const struct = Object.entries(value.values).map(([k, v]) => {
        const structValue = renderJavaScriptExperimentalValueNode(v);
        return {
    ...
    case 'enum':
      ...
      const enumValue = renderJavaScriptExperimentalValueNode(value.value);
    ...
    case 'optionSome':
      const child = renderJavaScriptExperimentalValueNode(value.value);
      return {
